...
긴 문제 설명에 비해 조금 간단한 문제였다. 각각 위치에 따라 칸들을 리스트로 분리하고, 리스트에서 숫자를 꺼내어 바구니 스택에 담는다. 담는 숫자와 바구니 스택 맨 위 숫자가 같으면 꺼내어 개수를 세어주면 터트려 없앤 개수가 나온다.

각 board를 각 위치에 따라 나누고, 크레인이 해당 리스트에서 꺼낼 때, 0이 아닌 숫자가 나올때까지 꺼내어주면 된다.
...
def solution(board, moves):
    ...
    stack = []
    board_list = [list(each_tuple) for each_tuple in list(zip(*board))]
    
    for move in moves:
        while board_list[move-1]:
            current = board_list[move-1]
            if current != 0:
