...
기존의 행렬 분해 방법과 item2vec, 딥러닝을 사용하여 각각의 추천 엔진 성능을 테스트 해보았다. 앱 종류는 대략 12만개, 사용자는 천만여개를 넘어가 일정 수준의 설치 수를 넘지 않는 어플리케이션이나 사용자를 제외하였고, sparse matrix를 만들어 사용하였다. 기본 Random 추천과 Popular 추천 mAP@10, 30, 50을 기준으로 놓고 다른 알고리즘 성능을 비교했다. Association Rule, Alternating Least Squares, item2vec + 기타 분류 알고리즘 혹은 딥러닝 등 성능을 오프라인에서 먼저 비교하였다.
